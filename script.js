...
// Auto-advance class code inputs + Enter key
codeInputs.forEach((box, index) => {
    box.addEventListener("input", () => {
        if (box.value.length === 1 && index < codeInputs.length - 1) codeInputs[index+1].focus();
    });
    box.addEventListener("keydown", e => { if(e.key==="Enter") submitCode(); });
});
