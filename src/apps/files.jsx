...
export default function Files({ state, setState, openWindow }){
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      <div style={{display:'flex',gap:8}}>
        ...
        <button onClick={()=> { const inp=document.createElement('input'); inp.type='file'; inp.onchange=(e)=>{/*no-op*/}; inp.click() }}>Upload</button>
      </div>
      <div style={{marginTop:8,overflow:'auto',flex:1}}>
        {state.files.map(f=> (
          <div key={f.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:8,borderRadius:8}}>
            <div style={{display:'flex',gap:12,alignItems:'center'}}><div>{f.type==='text'?'📄':f.type==='image'?'🖼️':'📦'}</div><div style={{minWidth:220}}>{f.name}</div></div>
            <div style={{display:'flex',gap:8}}>
