/* Main client JS — settings, UI, cursor, tools, iframe launcher
   All client-side. Saves preferences to localStorage.
*/
(() => {
  // ---- helpers ----
  ...
  (function particles(){
    ...
    let w,h,parts=[];
    function resize(){ w=c.width=innerWidth; h=c.height=innerHeight; parts=[]; for(let i=0;i<70;i++) parts.push({x:Math.random()*w,y:Math.random()*h,r:1+Math.random()*2,dx:(Math.random()-0.5)*0.6,dy:(Math.random()-0.5)*0.6}); }
    function frameF(){ ctx.clearRect(0,0,w,h); parts.forEach(p=>{ p.x+=p.dx; p.y+=p.dy; if(p.x<0)p.x=w; if(p.x>w)p.x=0; if(p.y<0)p.y=h; if(p.y>h)p.y=0; ctx.beginPath(); ctx.fillStyle='rgba(255,255,255,0.02)'; ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fill(); }); requestAnimationFrame(frameF); }
  ...
  if (cursorFile) cursorFile.addEventListener('change', ev => {
    ...
    const r = new FileReader(); r.onload = ()=>{ cursorData=r.result; save('cursor_data', cursorData); if(customCursor){ customCursor.src=cursorData; customCursor.style.width=(cursorSize.value||48)+'px'; customCursor.classList.remove('hidden'); } }; r.readAsDataURL(f);
  });
  if (cursorSize) cursorSize.addEventListener('input', e => { const v=+e.target.value; save('cursor_size', v); if(qs('#cursor-size-val')) qs('#cursor-size-val').textContent=v; if(customCursor) customCursor.style.width=v+'px'; });
  if (cursorClear) cursorClear.addEventListener('click', ()=>{ cursorData=null; save('cursor_data', null); if(customCursor) customCursor.classList.add('hidden'); if(cursorFile) cursorFile.value=''; });

  if ('ontouchstart' in window && customCursor) customCursor.classList.add('hidden');
  ...
  if (bgColor) bgColor.addEventListener('input', e => { const v=e.target.value; save('bg_color', v); document.body.style.background=`linear-gradient(180deg, ${v}, var(--bg-2))`; });
  ...
  // weather (offline-friendly)
  if (openWeather) openWeather.addEventListener('click', () => {
    openModal(`<h3>Weather (simple)</h3><div id="weather-info" style="margin-top:10px;color:var(--muted)">Attempting to fetch local weather...</div><div style="margin-top:10px"><input id="city-input" placeholder="Or enter a city" style="padding:8px"><button id="city-go" class="tool-btn">Go</button></div>`);
    const info = qs('#weather-info');
    if(navigator.geolocation){ navigator.geolocation.getCurrentPosition(pos => { const lat=pos.coords.latitude.toFixed(2), lon=pos.coords.longitude.toFixed(2); info.innerHTML = `Location: ${lat}, ${lon} <div class="small-muted">Enter city for mock result</div>`; }, err => { info.textContent='Location blocked. Enter a city below.'; }); } else info.textContent='Geolocation not available. Enter a city below.';
    qs('#city-go').addEventListener('click', ()=>{ const city = qs('#city-input').value.trim()||'Unknown'; info.innerHTML = `<strong>${city}</strong><div>${Math.floor(10+Math.random()*18)}°C — ${['Sunny','Cloudy','Light rain'][Math.floor(Math.random()*3)]}</div>`; });
  });
  ...
  // ---- cursor file input binding deferred earlier above ----

  // ---- load saved cursor size ----
  (function loadCursorSize(){ const cs = load('cursor_size', 48); if (cursorSize) cursorSize.value = cs; if (cursorSizeVal) qs('#cursor-size-val').textContent = cs; if (customCursor) customCursor.style.width = cs+'px'; })();

  // ---- modal escape key ----
  window.addEventListener('keydown', e => { if (e.key === 'Escape') { if (!modal.classList.contains('hidden')) closeModal(); else if (sidePanel && sidePanel.classList.contains('open')) { sidePanel.classList.remove('open'); logoBtn.classList.remove('open'); } } });

  // ---- small utilities ----
  function closeModal(){ if(modal) modal.classList.add('hidden'); setTimeout(()=>{ if(modalBody) modalBody.innerHTML=''; },300); }
  function openModal(html){ if(!modal) return; modalBody.innerHTML = html; modal.classList.remove('hidden'); }
